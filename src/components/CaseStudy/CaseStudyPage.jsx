...
/* Some agentsData.js entries name-check their own AIAgentsCasePage label
   inside a research/constraint body (e.g. "senseiq" saying "SenseIQ had to
   gracefully degrade..."). When that entry is reused under a different
   product name here, swap the label so the copy reads consistently —
   agentsData.js itself stays untouched since it's correct for its own page. */
function relabelStrategy(data, from, to) {
  const swap = (s) => s.split(from).join(to)
  const swapItem = (item) => (typeof item === 'string' ? swap(item) : { ...item, body: swap(item.body) })
  return {
    research: data.research.map(swapItem),
    constraints: data.constraints.map(swapItem),
    guardrails: data.guardrails.map(swap),
    variations: data.variations.map(swapItem),
  }
}
...
function StrategyContent({ data, accent = PALETTE[1], quadrant = false }) {
  return (
    <>
      ...
      <h4 style={T.h4}>Guardrails</h4>
      <UL>{data.guardrails.map((g) => <LI key={g}>{g}</LI>)}</UL>
      <h4 style={T.h4}>Design Variations Explored</h4>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '16px' }}>
        {data.variations.map((v, i) => (
          <InfoCard key={v.title} title={`${String.fromCharCode(65 + i)}. ${v.title}`} accent={accent}>{v.body}</InfoCard>
        ))}
      </div>
    </>
  )
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px', marginTop: '16px' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.9)', borderRadius: '14px', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.08)', fontFamily: "'Nunito', sans-serif", boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
              <div onClick={() => openSvgZoom(svgRefs[0])} style={{ background: '#f0ede8', borderBottom: '1px solid rgba(0,0,0,0.08)', padding: '0', cursor: 'zoom-in', transition: 'opacity 0.2s ease' }} onMouseEnter={e => e.currentTarget.style.opacity='0.88'} onMouseLeave={e => e.currentTarget.style.opacity='1'}>
                <AccordionCanvasMock ref={svgRef0} />
              </div>
...
function Step4() {
  ...
  return (
    <>
      ...
      <Section>
        <h3 style={T.h3}>1. Ask AI (The Workflow Assistant)</h3>
        <p style={T.body}><IH>The Problem:</IH> Ops managers building a workflow would get stuck mid-task — "How do I add a path node?", "What do blackout settings actually do?" — with no way to get an answer without leaving the canvas to search docs or ping a teammate.</p>
        <p style={T.body}><IH>The Solution:</IH> We built Ask AI as an embedded, contextual assistant inside the Workflow Canvas. A user types their question in plain English, and Ask AI answers with clear, step-by-step guidance scoped to exactly where they are — which they then follow directly in the same screen.</p>
      ...
      <Section>
        ...
        <CaseStudyAccordion items={[
          { title: 'Research & Design Decisions — Jarvis', content: <StrategyContent data={AGENTS.find((a) => a.id === 'data')} accent={PALETTE[5]} /> },
        ]} />
...
function Step5() {
  return (
    <>
      ...
      <Section>
        <h3 style={T.h3}>Step 1: Discover Agent (The Sourcer)</h3>
        <UL>
        ...
        <CaseStudyAccordion items={[
          { title: 'Research & Design Decisions — Discover Agent', content: <StrategyContent data={AGENTS.find((a) => a.id === 'matching')} accent={PALETTE[2]} /> },
        ]} />
        <CaseStudyImage src={asset('/illustrations/case-study/phase4/discover.png')} alt="Discover Agent" />
      </Section>

      <Section>
        <h3 style={T.h3}>Step 2: Voice Agent (The Screener)</h3>
        ...
        <p style={T.body}><IH>The Solution:</IH> I led the design of the <IH>Multimodal Agent Builder</IH> underneath the Voice Agent — a no-code interface that let "Architects" build Multimodal Chat+Voice Agents rather than rigid, single-channel bots.</p>
        <UL>
        ...
        <p style={T.body}>Built on this foundation, the Voice Agent itself handles the phone screen:</p>
        <UL>
        ...
        <CaseStudyAccordion items={[
          { title: 'Research & Design Decisions — Voice Agent', content: <StrategyContent data={AGENTS.find((a) => a.id === 'voice')} accent={PALETTE[0]} /> },
        ]} />
        <CaseStudyVideo src={asset('/videos/ai/voice-agent.mov')} />
      </Section>

      <Section>
        <h3 style={T.h3}>Step 3: Evaluation Agent (The Judge)</h3>
        <UL>
        ...
        <CaseStudyAccordion items={[
          { title: 'Research & Design Decisions — Evaluation Agent', content: <StrategyContent data={AGENTS.find((a) => a.id === 'screening')} accent={PALETTE[4]} /> },
        ]} />
        <CaseStudyImage src={asset('/illustrations/case-study/phase4/Evaluation Summary.png')} alt="Evaluation Agent" />
      </Section>

      <Section>
        <h3 style={T.h3}>Step 4: The Orchestrator — "Grace" (AI Recruiter)</h3>
        ...
        <CaseStudyAccordion items={[
          { title: 'Research & Design Decisions — Grace (Orchestrator)', content: <StrategyContent data={AGENTS.find((a) => a.id === 'ai-recruiter')} accent={PALETTE[3]} /> },
        ]} />
        <CaseStudyImage src={asset('/illustrations/case-study/phase4/HeroImage.png')} alt="Grace AI Recruiter" />
      </Section>

      <Section>
        <h3 style={T.h3}>Step 5: Solving "Auto-Submission" in the Agentic World</h3>
